  ...
  void _adicionarAoCarrinho() {
    ...
    final complementos = _complementosSelecionados.entries.map((entry) {
      ...
      return ComplementoSelecionado(
        produtoGrid: widget.produtoGrid,
    ...
    // Busca o nome do produto de várias fontes possíveis
    String nomeProduto = '';

    // 1. Tenta widget.nomeProduto (passado da tela anterior)
    if (widget.nomeProduto != null && widget.nomeProduto!.isNotEmpty) {
      nomeProduto = widget.nomeProduto!;
    }
    // 2. Tenta _produtoCompleto.produto.nome
    else if (_produtoCompleto!.produto.nome.isNotEmpty) {
      nomeProduto = _produtoCompleto!.produto.nome;
    }
    // 3. Tenta pegar do primeiro complemento (que tem produto_nome)
    else if (_produtoCompleto!.complementos.isNotEmpty) {
      final primeiroComp = _produtoCompleto!.complementos.first;
      nomeProduto = primeiroComp.produtoNome ?? '';
    }

    // Cria produto com dados corretos
    final produtoCorrigido = Produto(
      grid: widget.produtoGrid,
      nome: nomeProduto,
      descricao: _produtoCompleto!.produto.descricao,
      preco: _precoBase,
      precoPromocional: _produtoCompleto!.produto.precoPromocional,
      grupoGrid: _produtoCompleto!.produto.grupoGrid,
      grupoNome: _produtoCompleto!.produto.grupoNome,
      ativo: _produtoCompleto!.produto.ativo,
      controlaEstoque: _produtoCompleto!.produto.controlaEstoque,
      estoque: _produtoCompleto!.produto.estoque,
      unidade: _produtoCompleto!.produto.unidade,
      codigoBarra: _produtoCompleto!.produto.codigoBarra,
      imagemBase64: _produtoCompleto!.produto.imagemBase64,
    );
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            Text('${produtoCorrigido.nomeExibicao} adicionado!'),
          ],
        ),
